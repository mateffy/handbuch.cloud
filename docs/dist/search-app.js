...
async function getDb() {
  if (dbPromise)
    return dbPromise;
  dbPromise = (async () => {
    const workerUrl = new URL("/dist/sqlite.worker.js?v=2", window.location.origin).toString();
    const wasmUrl = new URL("/dist/sql-wasm.wasm?v=2", window.location.origin).toString();
